...
def register(request):
  ...
  if request.method == 'POST':
    ...
    if password == cpassword:
      ...
      Profile.objects.create(user=user, role_id=3, gender=gender, dob=dob, contact=contact, address=address, state_id=int(state), city_id=int(city), image=image ,password=password)
...
def email_send(request):
	...
	message = 'Forgetten password'
	email_from = settings.EMAIL_HOST_USER
	recipient_list = ['arvinderthakur232@gmail.com', ]
